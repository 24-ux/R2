...
{
	// 只缓冲数据，不发送
	obj->buf_position = Position;
	obj->buf_velocity = Velocity;
	obj->buf_KP = KP;
	obj->buf_KD = KD;
	obj->buf_torque = Torque;
...
HAL_StatusTypeDef DMsend_mit_data(DM_MotorModule *obj)
{
	...
	switch (obj->super_motor.mode){
		case POSITION:
			Postion_Tmp  =  float_to_uint(obj->buf_position,-P_MAX,P_MAX,16) ;
			Velocity_Tmp =  float_to_uint(obj->buf_velocity,-V_MAX,V_MAX,12);
			Torque_Tmp = float_to_uint(obj->buf_torque,0,0,12);
			KP_Tmp = float_to_uint(obj->buf_KP,0,500,12);
			KD_Tmp = float_to_uint(obj->buf_KD,0,5,12);
			break;
		case SPEED:
        ...
        case MIT:
            Postion_Tmp  =  float_to_uint(obj->buf_position,-P_MAX,P_MAX,16) ;
			Velocity_Tmp =  float_to_uint(obj->buf_velocity,-V_MAX,V_MAX,12);
			Torque_Tmp = float_to_uint(obj->buf_torque,-T_MAX,T_MAX,12);
			KP_Tmp = float_to_uint(obj->buf_KP,0,500,12);
			KD_Tmp = float_to_uint(obj->buf_KD,0,5,12);
            break;
	}
		
	obj->buf_tx_data[0] = (Postion_Tmp>>8);
	obj->buf_tx_data[1] = (Postion_Tmp);
	obj->buf_tx_data[2] = (uint8_t)(Velocity_Tmp>>4);
	obj->buf_tx_data[3] = (uint8_t)((Velocity_Tmp&0x0F)<<4) | (uint8_t)(KP_Tmp>>8);
	obj->buf_tx_data[4] = (uint8_t)(KP_Tmp);
	obj->buf_tx_data[5] = (uint8_t)(KD_Tmp>>4);
	obj->buf_tx_data[6] = (uint8_t)((KD_Tmp&0x0F)<<4) | (uint8_t)(Torque_Tmp>>8);
	obj->buf_tx_data[7] = (uint8_t)(Torque_Tmp);
	
	return HAL_FDCAN_AddMessageToTxFifoQ(&obj->super_motor.hcan, &tx_header, obj->buf_tx_data); 
}
                     ...
                     Ctrl_mode mode)
{
    ...
    obj->send_mit_data = DMsend_mit_data;
	...
	// 初始化缓冲数据
	obj->buf_position = 0.f;
	obj->buf_velocity = 0.f;
	obj->buf_KP = 0.f;
	obj->buf_KD = 0.f;
	obj->buf_torque = 0.f;
}
